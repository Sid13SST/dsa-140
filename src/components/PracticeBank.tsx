...
import { TIERS, type QTier, type SdQuestion } from '../data/sdPractice'
...
/**
 * Shared self-graded practice view. System design and AI/ML both work the same
 * way — attempt under a timer, reveal the rubric only afterwards, grade yourself
 * — so they share this component and differ in bank, rubric and copy.
 *
 * `rubricFor` is a prop because the two tracks have genuinely different
 * frameworks: system design grades sharding and back-of-envelope, AI/ML grades
 * labels, metrics and feedback loops.
 */
interface Props {
  ...
  eyebrow: string
  blurb: string
  /** Optional labels for the `family` field; when given, a family filter appears. */
  familyMeta?: Record<string, string>
}
...
function QuestionCard({
  ...
  const rubric = useMemo(() => rubricFor(q), [q, rubricFor])
...
export default function PracticeBank({
  ...
  const families = useMemo(
    () => (familyMeta ? Object.keys(familyMeta).filter((f) => bank.some((q) => q.family === f)) : []),
    [familyMeta, bank],
  )

  const attempted = bank.filter((q) => (progress[q.id]?.attempts ?? 0) > 0).length
  const strong = bank.filter((q) => {
    const a = progress[q.id]
    if (!a || a.attempts === 0) return false
    return a.hit.length / rubricFor(q).length >= 0.8
  }).length

  const visible = bank.filter(
    (q) => (tier === 'all' || q.tier === tier) && (family === 'all' || q.family === family),
  )
      ...
      <div className="card p-3">
        <div className="flex items-baseline justify-between gap-2 flex-wrap">
          ...
          <span className="font-mono text-[10px] text-muted">
            {bank.length} questions · 45 min each · self-graded
          </span>
        </div>
      ...
      <div className="card p-3">
        <div className="flex flex-wrap gap-2 mb-2">
          ...
          {TIERS.map((t) => (
            ...
            >
              {TIER_META[t].label} {bank.filter((q) => q.tier === t).length}
            </button>
          ))}
        </div>

        {families.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-2 pb-2 border-b border-rule/60">
            <button
              onClick={() => setFamily('all')}
              className={`btn text-xs ${family === 'all' ? 'btn-primary' : ''}`}
            >
              Every kind
            </button>
            {families.map((f) => (
              <button
                ...
                className={`btn text-xs ${family === f ? 'btn-primary' : ''}`}
              >
                {familyMeta![f]} {bank.filter((q) => q.family === f).length}
              </button>
            ))}
          </div>
        )}
        ...
        <ul className="divide-y divide-rule/60">
          {visible.map((q) => {
            const a = progress[q.id]
            const total = rubricFor(q).length
